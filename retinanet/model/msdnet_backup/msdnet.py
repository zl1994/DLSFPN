...
class MSDNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nBlocks):
            x = self.blocks[i](x)
            feature_after_block.append(x[0])

        feature_from_last_layer = x[1]
        feature_from_last_layer.append(x[0][-1])

        res = []
        for i in range(self.nBlocks):
            if len(feature_after_block[i])==2:
                feature_after_block[i].insert(0, feature_from_last_layer[1])
            elif len(feature_after_block[i])==1:
                feature_after_block[i].insert(0, feature_from_last_layer[2])
                feature_after_block[i].insert(0, feature_from_last_layer[1])
            res.append(tuple(feature_after_block[i][-3:]))
        return res
